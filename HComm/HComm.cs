...
namespace HComm {
    ...
    public class HComm {
        ...
        */
        ...
        /// </summary>
        public SendReceiveData SendReceiveMsg { get; set; }
        ...
        public bool GetParam(ushort addr, ushort count, bool merge = false) {
            ...
            try {
                ...
                // get item
                var item =
                    MsgQueue.Find(x => x.Command == Command.Read && x.Address == addr && x.Count == count);
                // check item
                if (item != null)
                    return false;
        ...
        /// <param name="slow">slow write</param>
        /// <returns>result</returns>
        public bool SetParam(ushort addr, ushort value, bool slow = false) {
            // check comm
            if (Comm == null)
                return false;
            // lock message queue
            ...
            try {
                ...
                if (MsgQueue.Count >= MaxQueueSize)
                    return false;
                // check count
                if (MsgQueue.Count < 3 || slow)
                    // add queue
                    MsgQueue.Add(new HCommMsg(Command.Write, addr, 1, Comm.PacketSetParam(addr, value)));
                else
                    // insert queue
                    MsgQueue.Insert(2, new HCommMsg(Command.Write, addr, 1, Comm.PacketSetParam(addr, value)));
        ...
        public bool GetInfo() {
            ...
            try {
                ...
                if (MsgQueue.Count >= MaxQueueSize)
                    return false;
                // check INFO queue
                if (MsgQueue.Find(x => x.Command == Command.Info) != null)
                    return false;
                // add queue
        ...
        public bool SetRealTime(ushort addr = 4002, ushort state = 1) {
            ...
            try {
                ...
                if (MsgQueue.Find(x => x.Command == Command.Mor && x.Address == addr && x.Count == state) != null)
                    return false;
                // add queue
        ...
        public bool SetGraph(ushort addr = 4100, ushort state = 1) {
            ...
            try {
                ...
                if (MsgQueue.Count >= MaxQueueSize)
                    return false;
                // check duplicate message
                if (MsgQueue.Find(x => x.Command == Command.Write && x.Address == addr && x.Count == state) != null)
                    return false;
                // add queue
                MsgQueue.Add(new HCommMsg(Command.Write, addr, state, Comm.PacketSetParam(addr, state)));
                // result
        ...
        public bool GetState(ushort addr = 3300, ushort count = 14) {
            ...
            try {
                ...
                if (MsgQueue.Count >= MaxQueueSize)
                    return false;
                // check duplicate message
                if (MsgQueue.Find(x => x.Command == Command.Mor && x.Address == addr && x.Count == count) != null)
                    return false;
                // check queue count
                if (MsgQueue.Count > 1)
                    // insert queue
        ...
        public bool GetGraph(ushort addr = 4200, ushort count = 1) {
            ...
            try {
                ...
                if (MsgQueue.Count >= MaxQueueSize)
                    return false;
                // check duplicate message
                if (MsgQueue.Find(x => x.Command == Command.GraphAd && x.Address == addr && x.Count == count) != null)
                    return false;
                // add queue
